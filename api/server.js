...
router.get("/", (req, res, next) => {
  ...
  db("accounts")
    ...
    .catch((err) =>
      res
        .status(500)
        .json({ errorMessage: "Failed to get accounts from the database" })
    );
});
...
router.post("/", (req, res, next) => {
  if (req.body.name && req.body.budget) {
    db("accounts")
      ...
      .catch((err) =>
        res.status(500).json({ errorMessage: "Failed to add data to database" })
      );
  } else {
    res
      .status(404)
      .json({ message: "name and budget are required properties" });
  }
});
